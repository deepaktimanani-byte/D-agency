  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <p className="text-body text-sm">{services.length} services</p>
        <Link
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              <th className="text-left px-5 py-3 font-semibold text-body text-xs uppercase tracking-wide hidden sm:table-cell">Featured</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-5 py-10 text-center text-body text-sm">No services yet. <Link href="/admin/services/new" className="text-navy underline">Add one</Link></td></tr>
            ) : (
              services.map((s) => (
                <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 hidden sm:table-cell">
                    {s.isFeatured && <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-navy/10 text-navy">Featured</span>}
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2 justify-end">
                      <Link href={`/admin/services/${s.id}`} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted hover:bg-gray-100 hover:text-navy transition-colors">
